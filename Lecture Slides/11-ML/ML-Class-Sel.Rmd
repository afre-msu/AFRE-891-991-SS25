...
author: "James Sears* <br> AFRE 891/991 SS25 <br> Michigan State University"
...
pacman::p_load(broom, data.table, DT, gridExtra,  FNN, glmnet, magrittr, tidyverse, viridis,
       tibble)
...
default_df <- ISLR::Default %>% 
  dplyr::mutate(i_default = 1 * (default == "Yes"))
```

---
# Classification Example

.hi-purple[The data:] The outcome, default, only takes .hi-blue[two values] (only `r default_df$i_default %>% mean() %>% scales::percent(accuracy = 0.1)` default).
...
.qa[A:] It depends...
--
 Remember that only `r mean(default_df$i_default) %>% scales::percent(accuracy = 0.01)` of the observations *actually* defaulted.
...
Function to Estimate KNN for given choice of K:
...
Running our function over k in 1:50,
...
ggplot() +
  geom_line(aes(x = k, y = value, linetype = "Training Error")) + 
...
Running KNN for $k=5$ on "truth":

```{r}
est_knn <- knn.reg(
  train = select(bayes_gen,x1, x2),
  test = bayes_truth,
  y = pull(bayes_gen, y),
  k = 5
)
...
and adding the predictions to the data:
```{r}
bayes_truth <- mutate(bayes_truth,
                      p = est_knn$pred,
                      y = as.numeric(est_knn$pred > 0.5))
```

---
# Bayes Decision Boundary

Sampling data points and estimating KNN for $k=5$ for .h-medgrn[two training data samples]:
.font90[
```{r}
# Sample data points
...
est_boundary <- knn.reg(
  train = select(bayes_sample, x1, x2),
  test = select(bayes_truth, x1, x2),
  y = bayes_sample$y,
  k = 5
)
est_boundary2 <- knn.reg(
  train = select(bayes_sample2, x1, x2),
  test = select(bayes_truth, x1, x2),
  y = bayes_sample2$y,
  k = 5
)
...
```{r, plot bayes boundary, eval = F, include = F, cache = T, dependson = "gen bayes data"}
...
ggsave("images/dec_bnd.png", device = "png")
```

```{r, echo = F}
include_graphics("images/dec_bnd.png")
```
...
Now we sample...
```{r, plot bayes sample, eval = F, include = F, cache = T, dependson = "gen bayes data"}
ggplot(data = bayes_truth, aes(x1, x2, color = y)) +
  ...
  theme_void() +
  theme(legend.position = "none")
ggsave("images/dec_bnd_samp.png", device = "png")
```

```{r, echo = F}
include_graphics("images/dec_bnd_samp.png")
```
---
# Bayes Decision Boundary

... and our first sample gives us an .hi-purple[estimated decision boundary].
```{r, echo = F, cache = F, dependson = "gen bayes data"}
ggplot(data = bayes_truth, aes(x1, x2, color = y)) +
  geom_point(shape = 20, size = 0.5) +
  geom_contour(
    aes(x = x1, y = x2, z = y),
...
```{r, plot bayes est boundary, eval = F, include = F, cache = T, dependson = "gen bayes data"}
ggplot(data = bayes_truth, aes(x1, x2, color = y)) +
  geom_point(shape = 20, size = 0.5) +
  geom_contour(
    aes(x = x1, y = x2, z = y),
  ...
  theme_void() +
  theme(legend.position = "none")
ggsave("images/dec_bnd_res.png", device = "png")
```

```{r, echo = F}
include_graphics("images/dec_bnd_res.png")
```
...
```{r, plot bayes est boundary 2, eval = F, include = F, cache = T, dependson = "gen bayes data"}
ggplot(data = bayes_truth, aes(x1, x2, color = y)) +
geom_point(shape = 20, size = 0.5) +
geom_contour(
  aes(x = x1, y = x2, z = y),
...
theme_void() +
theme(legend.position = "none")
ggsave("images/dec_bnd_est.png", device = "png")
```

```{r, echo = F}
include_graphics("images/dec_bnd_est.png")
```
...
```{r, plot knn k, eval = F, include = F, cache = T, dependson = "gen bayes data"}
ggplot(data = bayes_truth, aes(x1, x2, color = y)) +
geom_point(shape = 20, size = 0.5) +
geom_contour(
  aes(x = x1, y = x2, z = y),
...
theme_void() +
theme(legend.position = "none")
ggsave("images/dec_bnd_1_60.png", device = "png")
```

```{r, echo = F}
include_graphics("images/dec_bnd_1_60.png")
```
...
Historically, the go-to use of $k=10$ was fairly arbitrary. But what's the *right* $k$? Does the choice of $k$ even matter?
--

.pull-left[
.center.hi-slate[High K:] 
  * Model is trained on a .hi-medgrn[larger training set]
  * Tested on a .hi-red[smaller test set]
    * $\Rightarrow$ lower prediction error (in theory)
    * Computationally more intensive
]
--

.pull-right[
.center.hi-slate[Low K:] 
  * Model is trained on a .hi-red[smaller training set]
  * Tested on a .hi-medgrn[larger test set]
    * Theoretically higher prediction error, less computationally intensive
]

---
# Choice of K

.hi-slate[Solution: Multiple-k Repeated Cross-Validation]

  * Choose a variety of candidate $k$
    * Divisors of sample size are easiest
  * Repeat the CV for a given $k$ multiple times
    * i.e. repeat 10-fold CV 100 times, 5-fold CV 200 times
  * Look at median of observed error, choose best-performing $k$
    * Median to account for dependence across repeated CVs, underestimation of SE
--

What is the right range of $k$ to consider?
* "too large" $k$ imposes a finite number of sample combinations, limits how many iterations can be meaningfully different
      * LOOCV: $\left(\frac{n}{1} \right) = n = k$, and iteration makes no sense
* With large sample sizes, $\left(\frac{n}{k} \right) = \mathbf{BIG}$, *tons* of distinct samples and $k$ matters less
