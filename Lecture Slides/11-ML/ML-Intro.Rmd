...
pacman::p_load(broom, data.table, gridExtra, extrafont, furrr, future, magrittr, parallel, tidyverse, viridis,
       tibble)
...
that accurately describes $\color{#7BBD00}{\mathbf{y}}$ given some values of $\color{#6A5ACD}{\mathbf{x}_{1},\, \ldots,\, \mathbf{x}_{p}}$.

--

2. .hi-purple[Unsupervised learning] learns relationships and structure using only .hi-purple[inputs] $\left( \color{#6A5ACD}{\mathbf{x}_{1},\, \ldots,\, \mathbf{x}_{p}} \right)$ without any *supervising* output —letting the data "speak for itself."
...
We tend to further break .hi-medgrn[supervised learning] into two groups, based upon the .hi-green[type of output] (the type of outcome we want to predict):
...
* .hi-purple[Deep Learning (time permitting):] Can we predict attributes of a document or class of images?
...
1. .hi-dkgrn[Testing data]: The observations $(\color{#7BBD00}{y}_0,\color{#e64173}{x}_0)$ that our model .hi-purple[has yet to see] — and which we can use to .hi-purple[evaluate the performance] of $\hat{\color{#20B2AA}{f}}$.
...
1. .hi-blue[Cross validation] used to .hi-blue[estimate test error], evaluating performance or selecting a model's flexibility

--

1. .hi-red[Bootstrap] used to .hi-red[assess accuracy] of parameter estimates or methods
...
Two approaches we'll see:
  * .hi-purple[Leave-one-out cross-validation (LOOCV):] leave out .hi-purple[one observation], train the model, estimate error, repeat over all observations
  * .hi-pink[k-fold cross-validation:] split data into .hi-pink[k groups (folds)], leave out one fold and train the model, estimate error, repeat over all folds
