...
pacman::p_load(DiagrammeR, fixest, janitor, rsvg, DiagrammeRsvg, kableExtra,  huxtable, grf, magrittr, randomForest, 
               ranger, recipes, rpart.plot, rsample, scales, tidymodels, tidyverse, vip)
...
**Part 4: Deep Learning (if time - not a tree method)**
...
```{r}
# Define recipe
default_recipe <- recipes::recipe(default ~ ., data = default_df)
```
--
...
toc()
saveRDS(sim_x2_df, "C:/Users/searsja1/OneDrive - Michigan State University/Github/AFRE-891-991-SS25/Lecture Slides/11-ML/data/x2_sim_df.rds")
```
...
sim_x2_df <- readRDS("data/x2_sim_df.rds")
...
The original context for causal forests was in a .hi-medgrn[cross-sectional setting] with .hi-blue[random treatment assignment].
  * Assumes conditional independence between treatment assignment and outcome
  
How then can we extend into a typical .hi-purple[panel] setting]?
...
.hi-purple[1\. Group Effect]
  * [Jens, Page, and Reeder (2023)](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3907601)
  * Estimate the group-level fixed effects, use these for splitting in the forest
--
  
.hi-medgrn[2\. Difference-in-Differences Causal Forests (DiDCF)]
  * [Gavrilova, Langørgen, and Zoutman (2025)](https://www.dropbox.com/scl/fi/fbwr6g3zfqnecnkrz94h4/Gavrilova2025Payroll_and_Causal_Forest.pdf?rlkey=z78qbukzy1o9lhv6oe8bd4o8s&e=1&dl=0)
...
.hi-blue[3\. Causal Forests with Fixed Effects (CFFE)]
  * [Kattenberg, Scheer, and Thiel (2023)](https://www.cpb.nl/sites/default/files/omnidownload/CPB-Discussion-Paper-452-Causal-forests-with-fixed-effects-for-treatment-effect-heterogeneity-in-difference-in-differences.pdf)
  * Consistent CATT and ATT when common trend and no-anticipation hold at the subgroup level

  
---
# Manual Re-centering
Note that .hi-blue[manual re-centering] (i.e. demeaning variables before using them in causal forest) *doesn't* solve the temporal issue
  
  * If we demean by time, we use .hi-medgrn[all units] to identify that effect
  * $\Rightarrow$ re-centering isn't local, applies to *every unit*
  * $\Rightarrow$ identification fails if treatment effects are .hi-medgrn[heterogenous]... which is the whole point of using causal forests in the first place!
  
---
# Group Effect

The "Group Effect" approach of Jens, Page, and Reeder (2023) focuses on overcoming one dimension of limitation: .hi-purple[group-level heterogeneity].
--
...
Our goal becomes to estimate $\kappa$.

  * If $g(\cdot)$ is assumed to be linear, then we can use .hi-red[fixed effects from an OLS regression]
...
.hi-purple[1\. Build the Group Effect Variable]
...
And add the fixed effects as a .hi-medgrn[single variable] into the data:

```{r}
reg_group_fe <- data.frame(schoolid = names(fixef(reg_group)$schoolid) %>% as.numeric(),
...
Now adding the group effect variable into the $X$ matrix:
...
.pull-left[
```{r}
varimp_ge <- data.frame(covar = colnames(Xmat_ge),
                        imp = variable_importance(cf_ge)
                       ) %>%
  arrange(desc(imp))
