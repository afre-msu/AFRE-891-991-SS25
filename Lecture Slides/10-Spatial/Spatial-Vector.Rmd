...
.hi-slate[Solution:] either need to .hi-medgrn[compare to a units object] or .hi-blue[change the variable to numeric].

```{r name}
counties$NAME[which(counties$rail_dist > set_units(0, m))]

counties$NAME[which(as.numeric(counties$rail_dist) > 0)]
```
...
Our .hi-slate[choice of geometric predicate] will therefore influence *which* wellhead areas we select
...
ggplot() +
  ...
  geom_sf(data = whpa_within, fill = "navy", alpha = 0.6) + 
  maptheme
```
]
---
...
ggplot() +
  geom_sf(data = ingham, alpha = 0.5) +
  geom_sf(data = st_filter(whpa, ingham, .predicate = st_overlaps), fill = "maroon")  + 
  maptheme
...
```{r plotbox1, out.width = "50%", eval = T}
