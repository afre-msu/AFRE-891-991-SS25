...
pacman::p_load(exactextractr, prism, sf, terra, tidyterra, 
...
.hi-blue[1\. Local:] perform operations on a .hi-blue[cell-by-cell basis]

  * i.e. calculate difference between max and min daily temp for each cell

--

.hi-green[2\. Global:] apply bulk change to .hi-green[all cells in a raster]
...
.hi-purple[3 \. Focal:] create value using .hi-purple[neighboring cells' values]

  * i.e. average over current and contiguous cells

--

.hi-red[4\. Zonal:] apply math function to group of cells in a.hi-red[specific zone]
...
summary(temp_vals)
```
Returns a .hi-blue[matrix] with
* .hi-medgrn[One column per layer] (here tmin, tmax)
* .hi-purple[One row per cell]
...
* One row .hi-purple[per polygon]
* A single attribute column containing the .hi-red[polygon's value]
* Geometry column for the .hi-blue[original grid cell]
...
Plotting emphasizes that each .hi-medgrn[polygon] is the .hi-medgrn[original grid cell]:
...
.pull-left[
* `exactextractr`
  * Fast (1/4 the time of `terra`)
  * Works with many raster object types
  * Tons of options for progress, summary functions, + weighting
]
--
...
* `velox`
  ...
  * Much trickier to download/setup (no longer on CRAN)
]
...
The option `small = TRUE` makes sure values are extracted in the case that a polygon contains .hi-medgrn[no cell center points are within it].

  * Here it doesn't change anything
  
```{r extract_terra2, out.height = "70%"}
extr_terra2 <- terra::extract(tmax_crop, tracts, fun = mean,
                             small = TRUE ) #<< 

summary(extr_terra2$PRISM_tmax_stable_4kmD2_20230404_bil - extr_terra$PRISM_tmax_stable_4kmD2_20230404_bil)
```

---
# ExactExtractr

.hi-slate[ExactExtractr]'s `exact_extract()` is an even more flexible option:

  * Faster than `terra` (2-4 times at least)
  * By default considers partially covered pixels
  * Compatible with `RasterLayer`, `RasterStack`, `RasterBrick`, and `SpatRaster` objects
  * Options for weighting and using built-in and custom summary functions
  * Has built-in progress option (`progress = TRUE`)
    * Automatically gives you info about things that could cause bad performance
  * And many more ways to customize
  
---
# ExactExtractr

Using .hi-slate[ExactExtractr]'s `exact_extract()`:
 
```{r}
tic()
extr_ee <- exact_extract(tmax_crop, tracts, progress = FALSE)
toc(quiet = FALSE)
head(extr_ee)  
``` 
...
Woah okay we got something different this time!

```{r}
length(extr_ee)
extr_ee[[1]]
``` 
 * A list with one element per feature (586 rows in `tracts`)
 * Each list element is a data frame
  * Contains two variables: the cell value and how much of that cell is covered by the polygon


---
# ExactExtractr

If we specify our summary function as the mean this time, we get back a .hi-purple[vector]

  * Since there's only one layer, one summary function
...
We can force a .hi-red[data frame] with `force_df = TRUE`

  * One row per feature
  
```{r}
extr_ee <- exact_extract(tmax_crop, tracts, progress = FALSE,
                         fun = "mean", force_df = TRUE) #<<
head(extr_ee)
```
...
If we specify .hi-purple[multiple summary functions], we get back a .hi-red[data frame]:
...
| quantile | mode |
| majority | minority |
|variety (# distinct values)| (weighted) variance |
| (weighted) Std. Dev. | Coeff of Var.
...
For example, how many different max temp values are there within each census tract?

  
```{r}
extr_var <- exact_extract(tmax_crop, tracts, force_df = TRUE, progress = FALSE,
                         fun = "variety") #<<
head(extr_var)
```
---
...
Alternatively, define a custom summary operation

  * Average of cells at least 10% covered by the polygon
  
```{r}
mean_above_10p <- function(value, coverage_frac){ #<<
  mean(value[coverage_frac >= 0.1]) #<<
} #<<
extr_above_10p <- exact_extract(tmax_crop, tracts, progress = FALSE, force_df = TRUE,
                         fun = mean_above_10p) #<<
filter(extr_above_10p, result > 0) %>% head()
```
...
The [`velox`](https://hunzikp.github.io/velox/index.html) is the final package that can perform extractions a .hi-medgrn[lot faster] than `raster`
...
--
This speed comes  .hi-blue[at a cost], though:
...
```{r veloxraster, error = TRUE, eval = F}
pacman::p_load(velox)
