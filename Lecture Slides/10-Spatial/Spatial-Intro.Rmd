...
author: "James Sears <br> AFRE 891/991 SS 2% <br> Michigan State University"
...
Let's say you were interested in tracking fire occurrence points through the [US Forest Service's Monitoring Trends in Burn Severity data](https://apps.fs.usda.gov/arcx/rest/services/EDW/EDW_MTBS_01/MapServer).

Looking at the geodatabase folder doesn't show us anything intelligible
.center[
<img src="images/gdb.png" width = "50%"/>
]
...
Let's say you were interested in tracking fire occurrences and burn area over time through the [US Forest Service's Monitoring Trends in Burn Severity data](https://apps.fs.usda.gov/arcx/rest/services/EDW/EDW_MTBS_01/MapServer).

Checking the layers with `st_layers()`:

```{r}
st_layers("data/S_USA.MTBS_FIRE_OCCURRENCE_PT.gdb")
```
...
This one's easy with a single layer. Loading it in with `st_read()`:
  * We could've omitted the `layer` argument since there's just the one
  
```{r}
mtbs_area <- st_read("data/S_USA.MTBS_FIRE_OCCURRENCE_PT.gdb", 
                     layer = "MTBS_FIRE_OCCURRENCE_PT") #<<
```
...
# sf Geometry Types
There are three main types of geometries that can be associated with `sf` object: .hi-blue[points], .hi-medgrn[lines] and .hi-purple[polygons].
.center[
<img src ="images/pnt_line_poly.png" width="450"></img>]
---
...
We can easily convert an .hi-blue[sf object] to a .hi-pink[regular data frame] by .hi-medgrn[removing the geometry]
  * Convenient `st_drop_geometry()` function
...
We can also go from a .hi-pink[dataframe] to an .hi-blue[sf object]  by defining a geometry using column(s) containing coordinate information and `st_as_sf()`
...
Note that an sf object can have .hi-medgrn[multiple geometry columns], but only one can be
...
The .hi-slate[tmap] package makes is easy to make thematic maps in R, both .hi-medgrn[static] and .hi-purple[interactive]
...
CRSs are most commonly referenced by .hi-pink[EPSG codes], which you can Google.
...
- What you .hi-medgrn[want to preserve] (area, direction, or distance).
...
.hi-slate[Caution:] this only .hi-medgrn[changed the metadata], and .hi-blue[did not re-project the data]! The warning tells us as much:
...
To .hi-blue[*actually* transform (reproject)] the CRS, use `st_transform()`
...
**1. Web Mercator**
- Preserves .hi-blue[direction/angle/shape] but .hi-red[distorts area and distance].
...
- Preserves .hi-blue[area] but .hi-red[distorts direction/angle/shape and distance].
...
- .hi-red[Distorts everything]! Simply a graph of latitude vs. longitude.
