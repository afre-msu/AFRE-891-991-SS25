...
* Write .hi-red[custom functions] to execute specific tasks
  * Scrape all Yellowpages business links for a given search term in hundreds of different cities
--

* .hi-purple[Conditionally] define variables or execute different tasks
  * Create a variable conditional on another variables' values
--

* Perform a repeated task by .hi-pink[looping over values]
  * Create a set of state-level dummy variables from state FIPS codes
--

* Run tasks efficiently .hi-medgrn[in parallel]
...
Note that tidyverse has its *own, expanded* version of this function: `if_else()`. Benefits include

--

  * Can use the optional argument `missing` to handle NA values of the condition
...
--
  ...
  1. .hi-red[Vectorization and Functionals:] when you want to .hi-red[repeat a function] over .hi-purple[different values of arguments]
  
---
# For Loops

The .hi-blue[for loop] is a simple tool for .hi-blue[iteration]
...
```
  ...
  * `INDEX` the name of the index you want to use (often `i` but can be anything)
    ...
    * Often `i` but can be literally anything R-legal
--
  
  * `RANGE` the vector of values to iterate over 
    * Can be numbers, characters, or objects
--

  * `action` the stuff to do for each index value in the specified range
  
---
# For Loops

.hi-medgrn[Good use] for a for loop: repeat the same thing with minor tweaks.
...
Oftentimes we want our function code to span .hi-pink[multiple lines]. In this case we can use brackets around the function's operations:
...
Rather than write .hi-medgrn[anonymous] functions, .hi-blue[name your functions] to assign them to memory and reuse them throughout your file:
...
.footnote[<sup>2</sup> I want to note that this .hi-blue[isn't a useful function]. R's arithmetic functions already handle vectorised exponentiation and do so very efficiently.]
...
We can .hi-blue[specify return values] with `return()`
  * Helpful when our function performs a bunch of .hi-medgrn[intermediate steps]
...
If we left out the `return()`, the function will return .hi-pruple[the result of the very last operation]
  * Unless you assigned it to memory, in which case you get nothing
---
...
.hi-medgrn[1\. Return a List]

  
```{r}
square_ret_list <- function(x){ 
  x_sq <- x^2 # assign squared value as intermediate object 
...
We can also assign .hi-medgrn[default argument values]
  ...
  * Uses the supplied value when supplied
  * Uses provided default value when not
...
We get a similar error if we give the variable argument as a .hi-blue[data-variable] 
  * .hi-blue[data-variable]: a "statistical" variable that lives .hi-blue[in a data frame]
    * i.e. an object in the data frame (how we've been using variables in tidy verbs so far)
...
This is an issue of .hi-purple[indirection], which occurs in cases like this
  - Want to interpret the argument as an .hi-medgrn[environment-variable] rather than as a .hi-blue[data-variable].
...
.hi-purple[Solution A:] provide the argument as a .hi-blue[data-variable], then 
  1. .hi-purple[embrace] the argument with double braces `{{ arg }}` within function operations 
...
.hi-purple[Solution B:] provide the argument as a .hi-blue[data-variable], and 
  1. .hi-red[defuse] the string with `enquo()`
  
  1. .hi-dkgrn[inject] the defused string back in operations with `!!defused_string`

Let's work through the steps involved.

---
# Defusing and Injecting

.hi-red[Defusing] an argument with  .hi-red[delays its evaluation].
  * Use `enquo()` for defusing a single argmuent, `enquos()` for multiple simultaneously
  
For example, returning a defused variable highlights the different handling: 

```{r}
def_fun <- function(df, var){
  # defuse 
  var <- enquo(var) #<<
  return(var)
}
def_fun(murders, rate)
```

---
# Defusing and Injecting

When we're ready to do something with the data-variable, we .hi-dkgrn[inject] it back in
  * Add `!!` in front of the data-variable in the operation
  

```{r}
def_inj_fun <- function(df, var){
  # defuse 
  var <- enquo(var)
  res <- pull(df, !!var) #<<
  return(res)
}
def_inj_fun(murders, rate) %>% head()
```
...
.less-left[
Using the two in our square function involves
  1. Defuse the data-variable argument `var`
  1. Inject it in the squaring operation with `!!`
]
.more-right.font90[
```{r}
...
.hi-purple[Solution C:] defuse the argument with `ensym()`
...
`ensym()` and `enquo()` are two of the [tidy defusal operators](https://rlang.r-lib.org/reference/defusing-advanced.html). But what's the difference?
--

  * `ensym()` en"symbolizes" the argument and turns it into a .hi-pink[simple symbol].
--

  * `enquo()` converts the argument to a .hi-red[quosure]

--

When combined with `!!` they allow for pretty advanced defusal operations.

--

.hi-slate[Punchline:] in most cases, .hi-purple[embracing] with `{{ }}` .hi-purple[works in most cases] and is .hi-purple[simpler]

---

# Name Injection

We can combine defusing or embracing with .hi-purple[name injection] to customize our variable names.
...
Often we want to programmatically create new variable names based either on
 1. A .hi-medgrn[supplied character string] as a function argument, or
 1. .hi-blue[Modify the data-variable's name] directly in the function
...
.hi-medgrn[Approach 1:] use .hi-pink[glue syntax] and .hi-blue[supply the new name as a third argument]:
  * argument `newname` the new variable's name (as a character string)
...
.hi-medgrn[Approach 1:] use .hi-pink[glue syntax] and .hi-blue[supply the new name as a third argument]:
  ...
  * Glue syntax with `"{newname}"`
  * Programmatic assignment operator `:=` instead of `=`
  * quasi-quotes `{{ }}` evaluate the argument contents in the context of the data frame
...
.hi-medgrn[Approach 1:] use .hi-pink[glue syntax] and .hi-blue[supply the new name as a third argument]:
  ...
  * Glue syntax with `"{newname}"`
  * Programmatic assignment operator `:=` instead of `=`
  * quasi-quotes `{{ }}` evaluate the contents in the context of the data frame
...
.hi-medgrn[Approach 1] works with `ensym()` too 
...
.hi-blue[Approach 2A:] use .hi-pink[glue syntax] and .hi-blue[create the name from the data-variable]:
...
.hi-blue[Approach 2B:] use .hi-pink[glue syntax] and .hi-red[embracing]:
...
.hi-slate[Approach 2C:] you guessed it, `ensym()` still works, allowing the `var` argument to be *either* a data-variable or string.
```{r}
...
How long does it take to run this function?<sup>4</sup>
...
toc()
```
.footnote[<sup>4</sup> `sapply()` and `map()` take nearly the exact same time. There are also several [type-specific versions](https://www.rdocumentation.org/packages/purrr/versions/0.2.5/topics/map) of `map` in case you want output to be a logical, integer, double, or character, etc.]
...
To track progress in parallel,  we can use the  .hi-slate[progressr] package.<sup>5</sup>
...
.footnote[<sup>5</sup> When you're *not* in parallel, you can track progress just by adding the `progress = TRUE` argument in any of the`map()` functions.]
---
# Progress with *progressr*
...
```{r, eval = F}
sound_path <- paste0(getwd(), "/data/finish.wav")
handlers(list(
  "cli",
         handler_beepr(
