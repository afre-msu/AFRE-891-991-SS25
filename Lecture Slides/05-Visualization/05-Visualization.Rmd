...
Packages we'll use for today's examples:<sup>1</sup>
...
.footnote[<sup>1</sup> We'll also use .hi-slate[dslabs] later, but don't load it yet!]
...
To practice, let's load in a more complete version of the [gapminder data](https://www.gapminder.org/data/):
...
ca_fr <- filter(gapminder, country %in% c("Canada", "France")) %>%
  ggplot() +
...
gap_can <- filter(gapminder, country %in% c("Canada")) 
gap_fra <- filter(gapminder, country %in% c("France")) 
  ggplot() +
  geom_line(aes(x = year, y = life_expectancy), 
...
Add text + boxes with built-in spacing using `geom_label_repel()` from .hi-slate[ggrepel]

``` {r line8, out.width = "90%", eval = F}
...
``` {r line8_eval, out.width = "85%", echo = F}
# get a label x/y point for each country
g7_labs <- filter(gapminder, country %in% g7) %>%
  mutate(order = rep(seq(1:7), 57)) %>%
...
```{r, out.width = "70%"}
gapminder %>% mutate(gdp_percapita = gdp/population) %>%
  filter(year %in% c(1960, 2010) & !is.na(gdp_percapita), continent %in% c("Africa", "Americas")) %>%
  ggplot(aes(gdp_percapita, fill = continent)) + 
...
.small[
```{r, out.width = "66%"}
dens_df <- gapminder %>% mutate(gdp_percapita = gdp/population) %>%
  filter(year %in% c(1960, 2010) & !is.na(gdp_percapita))
